...
exports.validateCreateOrder = [
    ...
    (req, res, next) => {
        ...
        if (!errors.isEmpty()) {
            console.error(`[VALIDATE CREATE ORDER] Validation errors: ${JSON.stringify(errors.array())}`);
            return res.status(400).json({ errors: errors.array() });
        }
        console.log(`[VALIDATE CREATE ORDER] Validation passed: userId=${req.body.userId}, products=${JSON.stringify(req.body.products)}`);
        next();
    }
];
